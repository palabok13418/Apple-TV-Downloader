...
def main(args):
    ...
    def get_pssh(x):
        urlm3u8_request = requests.get(x, timeout=15).text
    ...
    def download_subs(filename, sub_url):
        m3u8_json = m3u8parser(requests.get(sub_url, timeout=15).text)
        ...
        SubtitleEdit_process = subprocess.run(
    [APTVConfig.SUBTITLE_EDIT, '/convert', f"{filename}.webvtt", "srt"],
    ...
    def m3u8_parsing(m3u8_url, timeeout=15):
        master_base_url = re.split('(/)(?i)', m3u8_url)
        del master_base_url[-1]
        master_base_url = ''.join(master_base_url)
        urlm3u8_master_request = requests.get(m3u8_url, headers=AUTH_HEADERS["wvHeaders"], timeout=15).text
    ...
    def m3u8_parser_alt(m3u8_url):
        ...
        del master_base_url[-1]
        master_base_url = ''.join(master_base_url)
        m3u8_master_json = m3u8parser(requests.get(m3u8_url, headers=AUTH_HEADERS["wvHeaders"], timeout=15).text)
        for media in m3u8_master_json['media']:
            audio_List = []
            if media['type'] == 'AUDIO':
                isAD = False
    ...
    def get_seasons(series_id):
        ...
        resp = requests.get(url=APTVConfig.APTV_ENDPOINTS["SERIES"] % (series_id), params=APTVConfig.APTV_PARAMS, cookies=COOKIES, timeout=15).json()
    ...
    def get_metadata(content_id):
        resp = requests.get(url=APTVConfig.APTV_ENDPOINTS["PRODUCT"] % (content_id), params=APTVConfig.APTV_PARAMS, cookies=COOKIES, timeout=15)#.json()
    ...
    def get_drm_info(id, titleName):
        resp = requests.get(url=APTVConfig.APTV_ENDPOINTS["CONTENT_DATA"] % (id), params=APTVConfig.APTV_PARAMS, headers=AUTH_HEADERS["wvHeaders"], cookies=COOKIES, timeout=15).json()['data']['content']
    ...
    def get_keys(pssh, content_info):
        cert_b64 = base64.b64encode(requests.post(url=APTVConfig.APTV_ENDPOINTS["WV_CERT"], headers=AUTH_HEADERS["wvHeaders"], timeout=15).content)
        ...
        license_rep = requests.post(url=APTVConfig.APTV_ENDPOINTS["WV_LICENSE"], json=licese_payload, headers=AUTH_HEADERS["wvHeaders"], timeout=15)
